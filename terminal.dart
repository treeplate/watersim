...
final File _logger = File('log.txt');
...
class Rect {
  ...
  @override
  String toString() => '$topLeft:$width*$height';
}
...
enum LineStyle { none, thin, bold, dotted, dottedBold, dashed, dashedBold, double, thickLeading, thickTrailing, filled }
...
class BracketFrame extends Widget {
  ...
  View createView(Screen screen) => BracketFrameView(this, screen);
...
class LabelView extends View {
  ...
  void updatePaint(Screen screen) {
    ...
    screen.drawText(rect.topLeft + Offset(0, widget.padding.top), _text, widget.foreground, widget.background);
...
class Label extends Widget {
  ...
  factory Label.strip({
    ...
    String line;
    switch (style) {
      case LineStyle.none: line = ' '; break;
      ...
      case LineStyle.filled: line = '\u2588'; break;
    }
    return Label(
...
class TextView extends View {
  TextView(Widget widget, Screen screen) : super(widget, screen);

  Text get widget => super.widget;
  ...
  void updateLayout() {
    ...
    while (_lines.length < rect.height) {
      _lines.add('');
    }
    for (int line = 0; line < _lines.length; line += 1) {
      switch (widget.textAlign) {
        case TextAlign.left:
          String text = _lines[line];
          text = text.substring(0, math.min(text.length, rect.width));
          _lines[line] = text.padRight(rect.width, ' ');
          break;
        case TextAlign.right:
          String text = _lines[line];
          if (text.length > rect.width) {
            text = text.substring(math.max(0, text.length - rect.width), text.length);
          }
          _lines[line] = text.padLeft(rect.width, ' ');
          break;
        case TextAlign.center:
          String text = _lines[line];
          text = text.substring(0, math.min(text.length, rect.width));
          double padding = (rect.width - text.length) / 2;
          String left = ' ' * padding.floor();
          String right = ' ' * padding.ceil();
          _lines[line] = '$left$text$right';
          break;
      }
    }
  }
  ...
  void updatePaint(Screen screen) {
    ...
    for (String line in _lines) {
      screen.drawText(rect.topLeft + Offset(0, index), line, widget.foreground, widget.background);
      index += 1;
    }
  }
}
...
class Text extends Widget {
  ...
  View createView(Screen screen) => TextView(this, screen);

  bool changesLayoutOf(Text widget) {
    return widget.text != text
        || widget.textAlign != textAlign;
  }

  bool changesPaintOf(Text widget) {
    return widget.foreground != foreground
        || widget.background != background;
  }
}
...
class BoxView extends View {
  BoxView(Widget widget, Screen screen) : super(widget, screen);

  Box get widget => super.widget;
  ...
  List<String> get _chars {
    switch (widget.style) {
      case LineStyle.none: return const <String>[' ', ' ', ' ', ' ', ' ', ' ', ' ', ' '];
      case LineStyle.thin: return const <String>['┌', '┐', '└', '┘', '─', '─', '│', '│'];
      case LineStyle.bold: return const <String>['┏', '┓', '┗', '┛', '━', '━', '┃', '┃'];
      case LineStyle.dotted: return const <String>['┌', '┐', '└', '┘', '┈', '┈', '┊', '┊'];
      case LineStyle.dottedBold: return const <String>['┏', '┓', '┗', '┛', '┉', '┉', '┋', '┋'];
      case LineStyle.dashed: return const <String>['┌', '┐', '└', '┘', '┄', '┄', '┆', '┆'];
      case LineStyle.dashedBold: return const <String>['┏', '┓', '┗', '┛', '┅', '┅', '┇', '┇'];
      case LineStyle.double: return const <String>['╔', '╗', '╚', '╝', '═', '═', '║', '║'];
      case LineStyle.thickLeading: throw UnimplementedError();
      case LineStyle.thickTrailing: throw UnimplementedError();
      case LineStyle.filled: return const <String>['▗', '▖', '▝', '▘', '▄', '▀', '█', '█'];
    }
    throw UnimplementedError();
  }

  void updateLayout() {
    child?.layout(const EdgeInsets.fromLTRB(1, 1, 1, 1).shrink(rect));
  }

  void updatePaint(Screen screen) {
    final List<String> chars = _chars;
    screen.drawText(rect.topLeft, _expandLine(chars[0], chars[4], chars[1]), widget.foreground, widget.background);
    if (rect.height > 1) {
      if (rect.height > 2 && rect.width > 0) {
        for (int y = 1; y < rect.height - 1; y += 1) {
          screen.drawText(rect.topLeft + Offset(0, y), chars[6], widget.foreground, widget.background);
          if (rect.width > 1) {
            screen.drawText(rect.topLeft + Offset(rect.width - 1, y), chars[7], widget.foreground, widget.background);
          }
        }
      }
      screen.drawText(rect.topLeft + Offset(0, rect.height - 1), _expandLine(chars[2], chars[5], chars[3]), widget.foreground, widget.background);
    }
  }

  String _expandLine(String left, String middle, String right) {
    if (rect.width == 0)
      return '';
    if (rect.width == 1)
      return left;
    int middleWidth = rect.width - 2;
    return '$left${middle * middleWidth}$right';
  }
}
...
class Box extends Widget {
  ...
  View createView(Screen screen) => BoxView(this, screen);

  void updateChildren(BoxView view, Screen screen) {
    view.child = screen.updateChild(view, view.child, child);
  }

  bool changesPaintOf(Box widget) {
    return widget.foreground != foreground
        || widget.background != background
        || widget.style != style;
  }
...
void log(Object x) {
  _logger.writeAsStringSync("$x\n", mode: FileMode.append);
}

void clearLog() {
  _logger.writeAsStringSync("");
}